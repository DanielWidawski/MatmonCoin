...
def create_trades_materilized_view(materilized_view_name: str, sample_by: str , TTL_time: str, refresh_rate: str, partition_by: str ) -> None:
   cur.execute(f""" CREATE MATERIALIZED VIEW {materilized_view_name} REFRESH EVERY {refresh_rate} AS (
...
with pg.connect(conn_str, autocommit=True) as connection:
     ...
     with connection.cursor() as cur:
       ...
       create_trades_materilized_view(materilized_view_name='ttttt', sample_by=sampled_by_hour, TTL_time="TTL 7 MONTH", refresh_rate = "1m", partition_by = "MONTH")
       #create_trades_materilized_view(materilized_view_name=sampled_by_day_materilized_view_name,sample_by=sampled_by_day, TTL_time="", refresh_rate = "10h", partition_by = "YEAR")
