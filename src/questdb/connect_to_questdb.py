...
with pg.connect(conn_str, autocommit=True) as connection:
     with connection.cursor() as cur:
       #create_binance_trades_materilized_view(materilized_view_name=sampled_by_day_materilized_view_name, sample_by=sampled_by_day)
       cur.execute(f"""SELECT * FROM {sampled_by_day_materilized_view_name}  
                    """)
